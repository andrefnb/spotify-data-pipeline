...
def test_transform_sorts_descending() -> None:
    ...
    album = Album(id="al1", name="AL", release_date="2024-01-01", total_tracks=1, uri="spotify:album:al1")
    track = Track(id="t1", name="T", duration_ms=1000, explicit=False, popularity=50, uri="spotify:track:t1", artists=[artist], album=album)

    older = PlayedAt(track=track, played_at=datetime(2024, 1, 1, tzinfo=timezone.utc))
    newer = PlayedAt(track=track, played_at=datetime(2024, 6, 1, tzinfo=timezone.utc))
